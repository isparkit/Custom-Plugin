...
<style type="text/css">
  .spanTshirt{border:1px solid #ccc;padding:3px 5px;border-radius:4px;margin-right:5px; font-size: 13px;}
  .pcamp{font-size:15px;line-height:1;}
  .div{-webkit-box-shadow: 0 3px 6px -6px #777;-moz-box-shadow: 0 3px 6px -6px #777;box-shadow: 0 3px 6px -6px #777; background: #f9f9f9; padding:15px; margin:15px 0px 15px 0px;}
  .adownload{border:1px solid #979595;padding:5px 10px;border-radius:4px;background:#ccc;text-decoration:none;color:#353535;font-weight:bold;margin:5px;}
  #listcamper li:nth-child(odd){background-color:#ebebeb;padding:10px;}
  #listcamper li:nth-child(even){background-color:#f2f7fb;padding:10px;}
  #listcamper{list-style:none;width:100%;text-align:left;font-size:15px;line-height:20px; text-transform: capitalize;}
</style>
<?php 
$searchorderid = 11086;
 $product_id = $_GET['id'];
    ...
    if(isset($_GET['yr'])){
      ...
      if($_GET['yr'] > '2022'){
        echo '<div  class="wrap">';
        require_once('camper-detail-new.php');
        echo '</div>';
      }
      else{
?>
<div  class="wrap">
  <div>
    <h1 class="wp-heading-inline">Camper Details</h1>
    <?php
      // order id
    ...
    $get_ordercamperid ="(SELECT bc.id, p.post_status, oim.meta_id, oim.order_item_id, oim.meta_value, oi.order_item_name as oi_camp, oi.order_id as oi_order_id,  pm.meta_value as user_id,  SUBSTRING_INDEX(oim.meta_value, ',', 1) as bookingcamper, SUBSTRING_INDEX(oim.meta_value, 'Shirt Size: ', -1) as osize,
    ...
    LEFT JOIN $wc_user_meta um ON um.user_id = pm.meta_value
    ...
    // camper emergendy 2 details  
                      ...
                      // include($plugin_dir.'/pdf-signin.php');
                      ?>
