...
function fetch_signup_data()
{
    ...
    $product_count = $loop->post_count;
        ...
        foreach($loop->posts as $pro)
        {
            ...
            // step1
            $get_orderid = "(SELECT DISTINCT p.order_item_id, pg.order_item_name, pg.order_id FROM $wc_item_meta p INNER JOIN $wc_items pg ON (pg.order_item_id = p.order_item_id) WHERE (p.meta_key = '_product_id' AND p.meta_value = $allirs))";
    ...
    bc.size, bc.bdate, bc.allergiescheck, bc.medidevicecheck, bc.allergies, bc.medidevice, bc.additional, bc.pfname, bc.plname, bc.relationship, bc.phone, bc.p2fname, bc.p2lname, bc.relationship2, bc.phone2, bc.photo, um.*,
    MAX(
    IF(
    ...
    WHERE oim.meta_key in( 'Camper 1', 'Camper 2', 'Camper 3', 'Camper 4', 'Camper 5', 'Camper 6', 'Camper 7', 'Camper 8', 'Camper 9', 'Camper 10' ) AND oim.order_item_id in($oitemids) AND p.post_status = 'wc-completed' AND oi.order_id $qoid  GROUP BY oim.meta_id )";
...
$output .= '<tr>';
$output .= '<td>'.$pro->post_title.'</td>';
$output .= '<td>';
if($total !=0){
for($i=0; $i<count($array_keys); $i++) {
    $output .= ' ['.$array_keys[$i] .': '.$array_values[$i].'] ';
}
}
else{
    $output .= ' 0 ';   
...
$output .= '</td>';
$output .= '<td align="right">'.$total.'</td>';
$output .= '</tr>';
}
return $output;
}
...
<table border="0">
  ...
  <tr>
   <td width="530">
      <h2>Total Camps: '.$product_count. '</h2>
    </td>
  </tr>
</table>
<br /><br />
<table border="1" cellpadding="3">
  <tr style="background-color:#C1E5FC;" >
    <th width="340"> Camp Name </th>
    <th width="410"> Order Details </th>
...
$pdfname = 'Total-Shirts_'.date("m-d-Y_h-i-sa").'.pdf';
$obj_pdf->Output($pdfname, 'D');
exit;
?>